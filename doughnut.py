...
fig, ax = plt.subplots()
ax.axis('equal')
mypie, _ = ax.pie(outsideSize, radius=1.3, colors=outsideColors)
plt.setp(mypie, width=0.3, edgecolor='white')

mypie2, _ = ax.pie(insideSize, radius=1, labeldistance=0.7, colors=intsideColors)
plt.setp(mypie2, width=0.4, edgecolor='white')
plt.margins(0, 0)

plt.title("Total Population Breakdown  61%s | %s | %s" % ("%", variant, year))
...
for i, p in enumerate(mypie):
    ...
    ax.annotate(outsideNames[i], xy=(x, y), xytext=(1.35*np.sign(x), 1.4*y),
